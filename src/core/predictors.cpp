...
inline void compute_trajectories_float(const PlanetarySystem* planet_system,
    ...
    for(uint i=0; i < predictor_steps; i++){
        ...
        for(it=planets.begin();it!=planets.end();it++){
            ...
            dmath::vec3 planet_origin = compute_planet_position(data, time);

            // iterate over each object
            for(uint j = 0; j < states.size(); j++){
                struct particle_state& current = states.at(i);
        ...
        position_buffers.at(i).push_back(state.origin.v[0] / scale);
        position_buffers.at(i).push_back(state.origin.v[1] / scale);
        position_buffers.at(i).push_back(state.origin.v[2] / scale);
...
inline void compute_trajectories_double(const PlanetarySystem* planet_system,
    ...
    position_buffers.reserve(states.size());
    for(uint i=0; i < states.size(); i++)
        position_buffers.push_back(std::vector<dmath::vec3>()); // no idea if this is correct - check when we compile lol 
        position_buffers.at(i).reserve(predictor_steps);
    ...
    for(uint i=0; i < predictor_steps; i++){
        ...
        position_buffers.at(i).push_back(state.origin / scale);
