...
void BaseApp::init(int gl_width, int gl_height){
    ...
    m_camera.reset(new Camera(dmath::vec3(-0.0f, 50.0f, 50.0f), 67.0f, (float)gl_width / (float)gl_height , 0.1f, 100000.0f, m_input.get()));
    ...
    m_bt_wrapper.reset(new BtWrapper(btVector3(0, -9.81, 0), &m_buffers, &m_thread_monitor, m_camera.get()));
